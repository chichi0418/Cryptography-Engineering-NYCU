...
# --- CORS 設定 (讓 GitHub Pages 可以呼叫 Render) ---
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # 為了方便測試先開全部，正式環境可設為 https://yourname.github.io
...
# --- WebAuthn 關鍵：如果前端在 GitHub，RP_ID 必須是 GitHub 的域名 ---
...
def get_expected_origins(request: Request) -> list[str]:
    # 如果有設定 RP_ID，則預期來源是該域名的 https 網址
    if RP_ID:
